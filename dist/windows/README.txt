...
1. Global Configuration:
   ...
   - Key settings:
     * default.output.dir - Where books are saved (default: output)
     * default.log.dir - Where logs are saved (default: logs)
     * default.temp.dir - Where temp files are stored (default: tmp)
     * default.output.format - Output format: epub or pdf (default: pdf)

2. Book-Specific Configuration:
   - Edit files in the config/ folder
   - Each .properties file configures one serie of books for one web manga
   - Paths in config files should use forward slashes (/) - they work on Windows
   - Example paths: output/my-book, tmp/my-book, logs/my-book
   - Add new book configurations by:
     a. Creating a new .properties file in config/
     b. Adding a reference to it in web2book.properties:
